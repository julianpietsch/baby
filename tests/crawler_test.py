...
def test_evolve_crawl(bb_evolve60, imgs_evolve60, tmp_path, save_crawler_output):
    ...
    for o, l in zip(output0, imgs_evolve60.keys()):
        ...
        if save_crawler_output and len(o['centres']) > 0:
            save_tiled_image(o['edgemasks'].transpose([1, 2, 0]).astype('uint8'),
                             tmp_path / '{}_t0_segoutlines.png'.format(l),
                             {k: o[k] for k in info_keys}, layout=(1, None))
